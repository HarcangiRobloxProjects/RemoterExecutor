...
static const char* unc_payload = R"LUA(
...
local Remoter = shared._rblx_Null
...
genv.identifyexecutor = make_c_closure(function() return "Remoter", "1.0.0" end)
...
genv.dumpstring = function(src)
    ...
    local ok, result = pcall(function()
        local resp = Remoter.request({Url = "http://localhost:19283/compile", Method = "POST", Body = src})
...
local old_loadstring = Remoter.loadstring
...
genv.http = {
    request = Remoter.request,
    get = function(url) return Remoter.request({Url=url, Method="GET"}).Body end,
    post = function(url, data) return Remoter.request({Url=url, Method="POST", Body=data}).Body end
}
