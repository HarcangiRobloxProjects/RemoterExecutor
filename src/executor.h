    ...
    static uintptr_t FindUnloadedModule(HANDLE hProcess, std::string& outName) {
        ...
        auto trySearchFolder = [&](uintptr_t folder) {
            ...
            for (uintptr_t child : rblx::GetChildren(hProcess, folder)) {
                ...
                // CRITICAL: Only target modules with jest/test/spec/story in their name
                // This avoids corrupting active UI modules which would crash the game.
                ...
                if (lowerName.find("jest") == std::string::npos && 
                    ...
                    lowerName.find("story") == std::string::npos) continue;
