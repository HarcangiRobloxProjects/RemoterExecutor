...
namespace RblxExecutorUI
{
    ...
    {
        [DllImport("Remoter.dll", CallingConvention = CallingConvention.StdCall)]
        ...
        [DllImport("Remoter.dll", CallingConvention = CallingConvention.StdCall)]
        public static extern ulong GetDataModel();
        ...
        [DllImport("Remoter.dll", CallingConvention = CallingConvention.StdCall)]
        public static extern int GetLastExecError([MarshalAs(UnmanagedType.LPStr)] System.Text.StringBuilder buffer, int bufLen);

        [DllImport("Remoter.dll", CallingConvention = CallingConvention.StdCall)]
        public static extern bool ReadMemory(ulong address, IntPtr buffer, uint size);

        [DllImport("Remoter.dll", CallingConvention = CallingConvention.StdCall)]
        public static extern bool WriteMemory(ulong address, IntPtr buffer, uint size);

        [DllImport("Remoter.dll", CallingConvention = CallingConvention.StdCall)]
        public static extern bool GetClientInfo([MarshalAs(UnmanagedType.LPStr)] System.Text.StringBuilder buffer, int maxSize);
    ...
    public class ScriptItem
    {
        ...
        public string DisplayIcon => IsCloud ? "M 1,12 C 1,8 5,5 8,5 C 10,2 14,2 16,5 C 19,5 23,8 23,12 C 23,16 19,19 16,19 L 8,19 C 5,19 1,16 1,12" : "M 2,2 L 10,2 L 14,6 L 14,14 L 2,14 Z M 10,2 L 10,6 L 14,6";
    }
    ...
    public partial class MainWindow : Window
    {
        ...
        private void MainWindow_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                Editor.Text = "-- Remoter Executor Premium\n-- Version: 1.0.0\n\nlocal player = game.Players.LocalPlayer\nprint(\"Hello, \" .. player.Name)";
        ...
        private void ExecuteScriptHub_Click(object sender, RoutedEventArgs e)
        {
            ...
            try 
            {
                ...
                if (btn != null && btn.Tag != null)
                {
                    ...
                    ScriptItem item = ScriptsList.FirstOrDefault(x => x.FullPath == path || x.CloudCode == path);

                    string content = "";
                    string title = "Script";

                    if (item != null && item.IsCloud)
                    {
                        content = item.CloudCode;
                        title = item.Name;
                    }
                    ...
                    {
                        Console.WriteLine($"[C# UI] Executing HUB script: {title} ({content.Length} chars)...");
                        ShowNotification($"Executing {title}...", false);
        ...
        {
            LoadScriptsFolder();
            ShowNotification("Refreshed Local Scripts", false);
        }

        private void ScriptSearchBox_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                ScriptBloxSearch_Click(sender, new RoutedEventArgs());
            }
        }

        private async void ScriptBloxSearch_Click(object sender, RoutedEventArgs e)
        {
            string query = ScriptSearchBox.Text;
            if (string.IsNullOrWhiteSpace(query))
            {
                ShowNotification("Enter a search term", true);
                return;
            }

            try
            {
                ShowNotification($"Searching ScriptBlox for '{query}'...", false);
                
                using (var client = new System.Net.Http.HttpClient())
                {
                    client.DefaultRequestHeaders.Add("User-Agent", "Remoter/1.0");
                    string url = $"https://scriptblox.com/api/script/search?q={Uri.EscapeDataString(query)}&max=20&mode=free";
                    ...
                    using (var doc = System.Text.Json.JsonDocument.Parse(response))
                    {
                        ...
                        {
                            string title = script.GetProperty("title").GetString() ?? "Unknown";
                            string code = script.GetProperty("script").GetString() ?? "";
                            ...
                            ScriptsList.Add(new ScriptItem
                            {
                                ...
                                IsCloud = true,
                                CloudCode = code,
                                FullPath = code // Use code as fallback tag
                            });
                        }
                    }
                }
                ...
                {
                    ShowNotification("No scripts found on ScriptBlox", true);
                    LoadScriptsFolder(); // Revert to local
                }
                else
                {
                    ShowNotification($"Found {ScriptsList.Count} scripts on ScriptBlox!", false);
                }
            }
            catch (Exception ex)
            {
                ShowNotification("ScriptBlox API Error", true);
                App.LogException(ex, "ScriptBloxSearch_Click");
                LoadScriptsFolder();
            }
        }
